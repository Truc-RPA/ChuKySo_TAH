...
import styles from "./VanBanBanHanh.module.scss";
import "@pnp/sp/webs";
...
import { IViewDoc30DaysProps } from "./IViewDoc30DaysProps";
import { IFolderInfo, IFileInfo } from "@pnp/sp/presets/all";
...
const ViewDocumentTree: React.FC<IViewDoc30DaysProps> = ({
  ...
  <div className={styles.container}>
    ...
    <div className={styles.contentBody}>
      {!(searchTerm.length > 0 && SearchTreeFolder) ? (
                    ...
                    <div className={styles.fileMeta}>
                      ...
                      {file.effectiveVersion && (
                        <div className={styles.metaItem}>
                          ...
                          <span>
                            <strong>Version:</strong> {file.effectiveVersion}
                          </span>
                        </div>
                      )}
                    </div>
        ...
        <div className={styles.contentBodyInner}>
          <div className={styles.fileList} style={{ maxWidth: "100%" }}>
            <div className={styles.fileHeader}>
              <h3>
                📄 Tài liệu tìm kiếm theo từ khóa "{searchTerm}" ({currentFolders.length} folders, {files.length} files)
              </h3>
            </div>
            <div>
              {!isLoading && folderHistory.length > 0 && (
              ...
              {!isLoading ? (
                currentFolders.map((folder: FolderItem) => (
                  <div key={folder.url} className={styles.documentItem} onClick={() => handleOpenFolder(folder)}>
                    <div className={styles.fileType}>
                      <FaFolder className={styles.iconFolders} />
                    </div>
                    <div className={styles.fileContent}>
                      <div className={styles.fileTitleRow} style={{ margin: 0 }}>
                        <div className={styles.fileTitle}>
                          <span className={styles.fileName}>{folder.name}</span>
                        </div>
                      </div>
                      <span style={{ fontSize: 10, color: "#6c757d" }}>
                        {folder.url.replace(`/${selectedLibrary}/`, "")}
                      </span>
                    </div>
                  </div>
                ))
            ...
            {!isLoading && (selectedFolder || (searchTerm && files.length > 0)) ? (
              paginatedFiles.length > 0 ? (
                paginatedFiles.map((file) => (
                  <div key={file.url} className={styles.documentItem} onClick={() => handleOpenFile(file.url, file.extension, file.name)}>
                    <div className={styles.fileHeader}>
...
export default ViewDocumentTree;